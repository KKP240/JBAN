...
const customerMiddleware = require("./src/middlewares/customerMiddleware");
...
app.get('/cart', authMiddleware, customerMiddleware, async(req, res) => {
    try {
...
app.get('/orderHistory', authMiddleware, customerMiddleware, async (req, res) => {
    try {
...
app.get("/custom_page", async (req, res) => {
    ...
    res.render("custom_page", { productId, productType, req });
});
...
app.get('/productdetails', async(req, res) => {
    ...
    try {
        ...
        res.render('productDetail', { product, reviews, req });
    } catch (error) {
        console.error(error);
        res.status(500).send('เกิดข้อผิดพลาดในการดึงข้อมูล');
    }
...
app.get('/favourite', authMiddleware, customerMiddleware, async(req, res) => {
    try {
...
app.get('/review', authMiddleware, customerMiddleware, async(req, res) => {
